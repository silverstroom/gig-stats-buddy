      ...
      <Table>
        ...
        <TableBody>
          {distribution.map((day, i) => {
            ...
            return (
              <TableRow key={day.date} className="border-foreground/[0.04] hover:bg-foreground/[0.02]">
                <TableCell className="font-bold text-xs">{day.day}</TableCell>
                <TableCell>
                  <div className="w-full bg-foreground/[0.04] rounded-full h-2.5 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${BAR_COLORS[i % BAR_COLORS.length]} transition-all duration-500 opacity-70`}
          ...
          <TableRow className="border-foreground/[0.06] bg-primary/[0.04] font-bold">
